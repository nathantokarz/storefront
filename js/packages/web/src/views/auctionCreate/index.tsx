...
const PriceAuction = (props: {
  ...
  const quoteMintName = props.attributes?.quoteMintInfoExtended?.name || "DINO"
          ...
          <label className="action-field">
            ...
            <Input
              ...
              placeholder={`Tick size in ${props.attributes.quoteMintInfoExtended? props.attributes.quoteMintInfoExtended.symbol
                : props.attributes.quoteMintAddress == WRAPPED_SOL_MINT.toBase58()? "SOL": "DINO"}`}
              prefix=""
              suffix={props.attributes.quoteMintInfoExtended? props.attributes.quoteMintInfoExtended.symbol
                : props.attributes.quoteMintAddress == WRAPPED_SOL_MINT.toBase58()? "SOL": "DINO"}
              onChange={info =>
...
const Congrats = (props: {
  ...
  const newTweetURL = () => {
    const params = {
      text: "I've created a new NFT auction on Dino Kingz Auction House, check it out!",
      url: `${
        window.location.origin
      }/#/auction/${props.auction?.auction.toString()}`,
      hashtags: 'NFT,Crypto,DinoKingz',
      // via: "Metaplex",
      related: 'NFT,Solana',
    };
